...
logger = logging.getLogger("label_perturbation_attack")

def label_flip_perturbation(X_train, X_test, X_val, y_train, y_test, y_val, change_unit):
    logger.info(f"label_flip_perturbation({X_train}, {X_test},{X_val},{y_train},{y_test},{y_val},{change_unit})")    
    try:
     S_p = y_train
...
#     print(S_p.shape)
     ...
     return S_p
        ...
        logger.error(f"label_flip_perturbation({X_train}, {X_test},{X_val},{y_train},{y_test},{y_val},{change_unit}) FAILED {Exception}")
        raise Exception
