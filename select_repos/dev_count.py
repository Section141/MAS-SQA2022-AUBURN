...
logger = logging.getLogger("dev_count")
...
def getDevEmailForCommit(repo_path_param, hash_):
    logger.info(f"getDevEmailForCommit({repo_path_param}, {hash_})")
    try:
     author_emails = []
     ...
     return author_emails  
        ...
        logger.error(f"getDevEmailForCommit({repo_path_param}, {hash_}) FAILED {Exception}")
        raise Exception
    
def getDevCount(full_path_to_repo, branchName='master', explore=1000):
    logger.info(f"getDevCount({full_path_to_repo}, {branchName},{explore})")
    try:
     repo_emails = []
     ...
     return len(repo_emails)
        ...
        logger.error(f"getDevCount({full_path_to_repo},{branchName},{explore}) FAILED {Exception}")
        raise Exception        
